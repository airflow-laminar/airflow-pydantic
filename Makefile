...
lint:  ## run python linter with ruff
	python -m ruff check airflow_pydantic
	python -m ruff format --check airflow_pydantic

# Alias
lints: lint

fix:  ## fix python formatting with ruff
	python -m ruff check --fix airflow_pydantic
	python -m ruff format airflow_pydantic
...
annotate:  ## run python type annotation checks with mypy
	python -m mypy ./airflow_pydantic
...
test:  ## run python tests
	python -m pytest -v airflow_pydantic/tests

coverage:  ## run tests and collect test coverage
	python -m pytest -v airflow_pydantic/tests --cov=airflow_pydantic --cov-report term-missing --cov-report xml
