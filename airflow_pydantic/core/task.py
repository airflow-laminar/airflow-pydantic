...
class TaskArgs(BaseModel, validate_assignment=True):
    ...
    email: list[str] | None = Field(default=None, description="the 'to' email address(es) used in email alerts")
    email_on_failure: bool | None = Field(default=None, description="Indicates whether email alerts should be sent when a task failed")
    email_on_retry: bool | None = Field(default=None, description="Indicates whether email alerts should be sent when a task is retried")
    retries: int | None = Field(default=None, description="the number of retries that should be performed before failing the task")
    retry_delay: timedelta | None = Field(default=None, description="delay between retries")
    retry_exponential_backoff: bool | None = Field(
        default=None,
    ...
    max_retry_delay: timedelta | None = Field(default=None, description="maximum delay interval between retries")
    start_date: DatetimeArg | None = Field(
        default=None,
    ...
    end_date: DatetimeArg | None = Field(default=None, description="if specified, the scheduler won’t go beyond this date")
    depends_on_past: bool | None = Field(
        default=None,
    ...
    # )
    execution_timeout: timedelta | None = Field(
        default=None,
    ...
    do_xcom_push: bool | None = Field(default=None, description="if True, an XCom is pushed containing the Operator’s result")
    multiple_outputs: bool | None = Field(
        default=None,
    ...
    task_display_name: str | None = Field(default=None, description="The display name of the task which appears on the UI.")
...
TaskAttribute = Union[str, tuple[str, str], tuple["Task", str]]  # noqa: UP007


class Task(TaskArgs, TaskRenderMixin, TaskInstantiateMixin, validate_assignment=True):
    task_id: str | None = Field(default=None, description="a unique, meaningful id for the task")

    operator: ImportPath = Field(description="airflow operator path")
    dependencies: list[TaskReference] | list[TaskAttribute] | None = Field(default=None, description="dependencies")
        ...
        for i, element in enumerate(v):
            if isinstance(element, tuple):
                ...
                if not isinstance(element[1], str):
                    raise TypeError("The second element of the tuple must be a string representing the attribute name.")
