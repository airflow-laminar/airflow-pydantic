...
class DagArgs(BaseModel, validate_assignment=True):
    ...
    description: str | None = Field(default=None, description="The description for the DAG to e.g. be shown on the webserver")
    schedule: ScheduleArg | None = Field(
        default=None,
    ...
    start_date: DatetimeArg | None = Field(default=None, description="The timestamp from which the scheduler will attempt to backfill")
    end_date: DatetimeArg | None = Field(default=None, description="A date beyond which your DAG won’t run, leave to None for open-ended scheduling")
    ...
    default_args: TaskArgs | None = Field(default=None, description="Default arguments for tasks in the DAG")
    ...
    max_active_tasks: int | None = Field(default=None, description="the number of task instances allowed to run concurrently", gt=0)
    ...
    default_view: Literal["grid", "graph", "duration", "gantt", "landing_times"] | None = Field(
        default=None,
        description="Specify DAG default view (grid, graph, duration, gantt, landing_times), default grid",
    )
    orientation: Literal["LR", "TB", "RL", "BT"] | None = Field(
        default=None, description="Specify DAG orientation in graph view (LR, TB, RL, BT), default LR"
    )
    catchup: bool | None = Field(default=None, description="Perform scheduler catchup (or only run latest)? Defaults to False")
    ...
    is_paused_upon_creation: bool | None = Field(
        default=None,
    ...
    tags: list[str] | None = Field(default=None, description="List of tags to help filtering DAGs in the UI.")
    ...
    dag_display_name: str | None = Field(default=None, description="The display name of the DAG which appears on the UI.")

    # Extras
    enabled: bool | None = Field(default=None, description="Whether the DAG is enabled")
    ...
    def _validate_params(cls, v):
        ...
        if isinstance(v, PydanticBaseModel) or (isinstance(v, type) and issubclass(v, PydanticBaseModel)):
            ...
            for key, value in new_v.items():
                ...
                if hasattr(resolved_type, "__name__") and resolved_type.__name__ == "Annotated":
                    # If the type is Annotated, we need to extract the inner type
                    resolved_type = resolved_type.__args__[0]
                if hasattr(resolved_type, "__name__") and resolved_type.__name__ == "Literal":
                    # If the type is Literal, we need to extract the inner type
                    resolved_type = type(resolved_type.__args__[0])
                if (hasattr(resolved_type, "__name__") and resolved_type.__name__ == "Union") or isinstance(resolved_type, UnionType):
...
class Dag(DagArgs, DagRenderMixin, DagInstantiateMixin, validate_assignment=True):
    ...
    tasks: dict[str, SerializeAsAny[Task]] | None = Field(default_factory=dict, description="List of tasks in the DAG")
