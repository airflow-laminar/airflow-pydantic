...
if _airflow_3():
    ...
    def get_pool(pool_name: str, *args, **kwargs) -> Pool:
        """Get a pool by name via the LocalRESTClient."""
        import httpx

        try:
            client = get_local_rest_client(process_type="dag_processor")
            resp = client.pools.get(pool_name)
            return Pool(
                pool=resp.pool,
                slots=resp.slots,
                description=resp.description or "",
                include_deferred=resp.include_deferred,
            )
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 404:
                raise PoolNotFound(f"Pool {pool_name} not found") from e
            raise

    def create_or_update_pool(name: str, slots: int = 0, description: str = "", include_deferred: bool = False, *args, **kwargs):
        """Create or update a pool via the LocalRESTClient."""
        import httpx

        client = get_local_rest_client(process_type="dag_processor")
        try:
            resp = client.pools.update(name, slots=slots, description=description, include_deferred=include_deferred)
        except httpx.HTTPStatusError:
            resp = client.pools.create(name=name, slots=slots, description=description or "", include_deferred=include_deferred)
        return Pool(
            pool=resp.pool,
            slots=resp.slots,
            description=resp.description or "",
