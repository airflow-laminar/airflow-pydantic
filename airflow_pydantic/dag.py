...
class Dag(DagArgs):
    ...
    def render(self) -> str:
        ...
        for task_id, task in self.tasks.items():
            task_imports, task_globals, task_code = task.render(dag_from_context=True)
        ...
        rendered_code = ["# Generated by airflow-config"] + sorted(["from airflow.models import DAG"] + imports) + ["", ""]

        # Globals
        if globals_:
            globals_ = list(set(globals_))  # Remove duplicates
            rendered_code.extend(["# Globals"] + sorted(globals_) + ["", ""])

        # DAG definition
        rendered_code.append(f"with DAG(dag_id='{self.dag_id}') as dag:")

        # Tasks
        rendered_code.append("    # Tasks")
        for task_id, task in tasks.items():
            rendered_code.append(f"    {task_id} = {task}")
        rendered_code.append("")
        return "\n".join(rendered_code)
